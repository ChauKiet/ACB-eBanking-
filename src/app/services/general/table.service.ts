...
export class TableService {
	...
	_ini(option) {
		let item = { skip: false, url: null, count: 40, counts: [10, 20, 40, 50, 100], pages: 1, textpages: null, key: "id" };
	...
	processReturn(data) {
		...
		//console.log(first + ' __ ' + last + '_|__' + total + '____' + this.settings['count'])
		...
		if (this.isLoad == false) {
			...
			for (let i = 0; i < this.cols.length; i++) {
				if (this.cols[i]['field'] == field && this.cols[i]['filter'] == true) {
					this.sorting['sort'] = (this.sorting['sort'] == 'asc') ? "desc" : "asc";
